...
export default function InsumoManager() {
  ...
  // --- CORRECCIÓN: Filtrar eliminados antes de aplicar el buscador ---
  // Primero filtramos los que NO están eliminados
  const insumosActivos = insumos.filter(
    (i) => i.estado !== 'ELIMINADO'
  );

  // Luego aplicamos el buscador sobre los activos
  const filtroNormalizado = normalizar(filtro);
  const insumosFiltrados = insumosActivos.filter((a) => {
  ...
  const totalPaginas = Math.ceil(
    insumosFiltrados.length / registrosPorPagina
  );
    ...
    <section>
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <h2 className="text-2xl font-bold text-gray-700 flex items-center">
          <PlusCircle className="mr-2 text-green-600" size={26} />{' '}
      ...
      <div className="overflow-x-auto bg-white rounded-2xl shadow-md border border-gray-200">
        <table className="min-w-full text-xs md:text-sm text-left border-separate border-spacing-y-1">
          <thead className="bg-gray-100">
            <tr>
              <th className="p-3 font-semibold text-gray-700 rounded-tl-2xl">
                Nombre
              </th>
              ...
              <th className="p-3 font-semibold text-gray-700 rounded-tr-2xl">
                Acciones
              </th>
            </tr>
          </thead>
          <tbody>
            {loadingInsumos ? (
              ...
              insumosPagina.map((a) => (
                <tr
                  key={a.insumoId}
                  className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg hover:bg-green-50 transition-all"
                >
                  <td className="p-3 font-bold text-gray-800 align-middle rounded-l-xl">
                    {a.nombre}
                  </td>
                  ...
                  <td className="p-3 text-green-700 font-semibold align-middle">
                    {a.energiaKcal ?? 0}
                  </td>
                  <td className="p-3 text-blue-700 font-semibold align-middle">
                    {(a.proteinaAnimalG ?? 0) +
                      (a.proteinaVegetalG ?? 0)}
                  </td>
                  <td className="p-3 text-yellow-700 font-semibold align-middle">
                    {a.choCarbohidratoG ?? 0}
                  </td>
